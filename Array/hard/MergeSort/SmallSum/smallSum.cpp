namespace small_sum
{
	...
	int merge(vector<int>& nums, int left, int mid, int right)
	{
		...
		{
			// 左组严格小于右组数之时，右组中有多少个(right - r + 1)数严格大于此时左组当前数nums[l]的，进行累加
			ret += nums[l] < nums[r] ? (right - r + 1) * nums[l] : 0; 
			tmp[i++] = (nums[l] < nums[r] ? nums[l++] : nums[r++]);
			/*if (nums[l] < nums[r])
			{
				tmp[i] = nums[l];
				ret += (right - r + 1) * nums[l];
				l++;
			}
